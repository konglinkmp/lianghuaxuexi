...
- 🛡️ **智能风控**：分级回撤控制、月度回撤管理、AI 新闻风险分析、**行业集中度控制、持仓相关性预警**
- 🧠 **专家见解集成**：支持情绪因子干预、中小盘诱多风险识别、5日线回踩买入建议
- � **动态适配**：**风格漂移监控（RPS）**，自动识别大/小盘占优并切换防守/进攻模式
- ⚡ **执行增强**：**集合竞价异常检测、尾盘跳水监控**
...
# 查看生成的交易计划
open data/trading_plan.md  # Mac
# 或
cat data/trading_plan.md   # 命令行查看
...
| 竞价过滤 | 开盘前过滤跳空及异常波动 | `PYTHONPATH=src python -m quant.main --auction-only` |
| 风格监控 | 识别市场风格漂移 (RPS) | 自动集成在主程序中 |
| 组合风控 | 检查行业集中度与相关性 | 自动集成在生成计划中 |
...
# 💡 进阶：集成专家见解（如博主建议谨慎）
PYTHONPATH=src python -m quant.main --sentiment -0.5
...
- 终端显示分层推荐股票
- 终端显示分层推荐股票
- 自动保存到 `data/trading_plan.md` 报告文件（含操作备注、AI风险提示、行业/概念信息）

**查看结果：**
```bash
# 用浏览器或 Markdown 阅读器打开
open data/trading_plan.md

# 或用命令行查看
cat data/trading_plan.md
```
